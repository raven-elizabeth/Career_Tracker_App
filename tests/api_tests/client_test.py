...
class ClientTest(unittest.TestCase):
    ...
    @patch("data_access.api_client.client.requests.get")
    def test_get_entry_by_date_returns_entry(self, mock_get):
        """Test that get_entry_by_date returns a DailyEntry object when the API responds with a valid entry."""
        # Arrange: Configure the mock to return a 200 response with the sample entry inside a "data" key
        mock_get.return_value = MagicMock(status_code=200, json=lambda: {"data": SAMPLE_ENTRY})
    ...
    def test_get_entry_by_date_unexpected_error_raises(self, mock_get):
        """Test that get_entry_by_date raises a ValueError when the API responds with an unexpected error status."""
        # Arrange: Simulate a 500 server error — any non-200/404 status should raise a ValueError
    ...
    @patch("data_access.api_client.client.requests.post")
    ...
    def test_save_entry_failure_raises(self, mock_post):
        """Test that save_entry raises a ValueError when the API responds with an error status."""
        # Arrange: Simulate a 400 Bad Request — e.g. missing required fields
        mock_post.return_value = MagicMock(status_code=400, json=lambda: {"error": "Bad request"})

        # Act & Assert
        with self.assertRaises(ValueError):
            ApiClient().save_entry(SAMPLE_ENTRY)

    @patch("data_access.api_client.client.requests.put")
    def test_replace_entry_returns_data(self, mock_put):
        """Test that replace_entry returns the updated entry data when the API responds with a 200 OK."""
        # Arrange: Simulate a successful 200 OK response after a full replacement
        mock_put.return_value = MagicMock(status_code=200, json=lambda: {"data": SAMPLE_ENTRY})
    ...
    def test_replace_entry_not_found_raises(self, mock_put):
        """Test that replace_entry raises a ValueError when the API responds with a 404 Not Found."""
        # Arrange: Simulate a 404 — trying to replace an entry that does not exist
        mock_put.return_value = MagicMock(status_code=404, json=lambda: {"error": "Not found"})

        # Act & Assert
        with self.assertRaises(ValueError):
            ApiClient().replace_entry(SAMPLE_ENTRY)

    @patch("data_access.api_client.client.requests.patch")
    def test_update_entry_returns_data(self, mock_patch):
        """Test that update_entry returns the updated entry data when the API responds with a 200 OK."""
        # Arrange: Simulate a successful 200 OK response after a partial update
    ...
    def test_update_entry_failure_raises(self, mock_patch):
        """Test that update_entry raises a ValueError when the API responds with an error status."""
        # Arrange: Simulate a 400 Bad Request — e.g. all fields are empty
        mock_patch.return_value = MagicMock(status_code=400, json=lambda: {"error": "Bad request"})

        # Act & Assert
        with self.assertRaises(ValueError):
            ApiClient().partially_update_entry(SAMPLE_ENTRY)

    @patch("data_access.api_client.client.requests.delete")
    def test_delete_entry_succeeds(self, mock_delete):
        """Test that delete_entry completes without raising when the API responds with a 204 No Content."""
        # Arrange: Simulate a successful 204 No Content response — deletion has no response body
        ...
        # Assert
        # Added this myself - AI did not include an assert as just wanted to check no exception is raised,
        # but it's good to be explicit about the expected return value
        self.assertEqual(None, response)

    @patch("data_access.api_client.client.requests.delete")
    def test_delete_entry_not_found_raises(self, mock_delete):
        """Test that delete_entry raises a ValueError when the API responds with a 404 Not Found."""
        # Arrange: Simulate a 404 — trying to delete an entry that does not exist
        mock_delete.return_value = MagicMock(status_code=404, json=lambda: {"error": "Not found"})
