"""
This screen allows users to create a new entry or edit an existing one by
selecting a date. It features a dropdown for date selection, dynamically
generated fields based on the FIELDS list.
...
class NewEntryScreen(Screen):
    ...
    # Number of days to show in the date dropdown (today + past year)
    DATE_RANGE_DAYS = 365
    # Width of the date Combobox widget in characters
    DATE_DROPDOWN_WIDTH = 20

    # Vertical padding for field labels — multiline labels need 14px top
    # padding (vs the standard SMALL_PAD) so the label aligns with
    # the first line of the Text widget rather than the widget's centre
    MULTILINE_LABEL_PAD_Y = (14, Screen.SMALL_PAD)

    # Bottom padding for the save-button container row
    SAVE_BUTTON_PAD_Y = (Screen.SMALL_PAD, Screen.SCREEN_EDGE_PADDING)
    ...
    def __init__(self, *args, client, on_home, **kwargs):
        ...
        # Track whether we're editing an existing entry (True) or creating
        # a new one (False). This determines which API route is used on save.
        self._editing = False
    ...
    def _configure_grid(self):
        """Single column; header, date, fields, and save button stack
        vertically."""
    ...
    def refresh_screen(self, event=None, date=None):
        """Reset the screen for the given date, defaulting to today if none
        is provided.
        ...
        - event= provided (ComboboxSelected binding): tkinter has already
          updated _selected_date, so just read it without overwriting.
        """
        # Always reset editing state — the entry for the new date is checked
        # fresh below
        self._editing = False
        self._original_data = None
    ...
    def _on_edit(self, original_data):
        """Set up the screen for editing an existing entry by prepopulating
        fields and tracking original data."""
    ...
    def _setup_title(self):
        ...
        self.title_label.grid(
            row=0, column=0,
            padx=(self.SCREEN_EDGE_PADDING, self.SMALL_PAD),
            pady=(self.INNER_FRAME_PADDING, self.SMALL_PAD),
            sticky="w",
        )
    ...
    def _setup_back_button(self):
        ...
        self._position_button(
            ...
            pad_x=(self.SMALL_PAD, self.SCREEN_EDGE_PADDING),
            pad_y=(self.INNER_FRAME_PADDING, self.SMALL_PAD),
            sticky="e",
        )

    def _setup_date_frame(self):
        """Create a centred date label and dropdown for selecting the entry
        date."""
        date_frame = Frame(self, bg=self.PRIMARY_COLOR)
        date_frame.grid(
            row=1, column=0, columnspan=2,
            padx=self.SCREEN_EDGE_PADDING, pady=(0, self.SMALL_PAD),
            sticky="ew",
        )
        ...
        Label(
            ...
            fg=self.SECONDARY_COLOR,
            bg=self.PRIMARY_COLOR,
        ).grid(row=0, column=0, pady=(0, self.SMALL_PAD // 2))

        self._setup_date_dropdown(date_frame)

    def _setup_date_dropdown(self, parent):
        """Create a dropdown with options for today and the past year,
        defaulting to today."""
        ...
        # Combobox requires StringVar to track selected value; default to
        # today's date and use readonly to prevent invalid input
    ...
    def _setup_entry_data(self, parent):
        for i, field in enumerate(ENTRY_FIELDS):
            ...
            # Labels next to a tall Text widget are pinned to the top-left
            # with matching top padding so they align with the first line of
            # text. Labels next to a single-line Entry are truly vertically
            # centred with sticky="w".
            label_sticky = "nw" if is_multiline else "w"
            label_pad_y = (
                self.MULTILINE_LABEL_PAD_Y if is_multiline
                else self.SMALL_PAD
            )
            ...
            Label(
                ...
                row=i, column=0,
                padx=(self.INNER_FRAME_PADDING, self.SMALL_PAD),
                pady=label_pad_y,
                sticky=label_sticky,
            )

            widget = self._create_text_widget(
                parent=parent, multiline=is_multiline
            )
            widget.grid(
                row=i, column=1,
                padx=(0, self.INNER_FRAME_PADDING),
                pady=self.SMALL_PAD,
                sticky="ew",
            )
    ...
    def _create_text_widget(self, parent, multiline=False):
        """Helper function to create either a Text or Entry widget based on
        the multiline parameter."""
        if multiline:
            return Text(
                parent,
    ...
    def _setup_save_button(self):
        ...
        save_btn.grid(padx=self.SCREEN_EDGE_PADDING)

    def _input_is_valid(self, raw_data):
        """Validate that at least one field is filled and no fields exceed
        character limits before allowing save."""
        valid = True
        if not raw_data or not any(
            raw_data[field] for field in ENTRY_FIELDS
        ):
            self._show_error(
                "No input detected",
        ...
        for field, value in raw_data.items():
            if len(value) > self.CHARACTER_LIMIT:
                self._show_error(
                    f"Input for {field.replace('_', ' ')} too long",
                    f"Please limit each field to "
                    f"{self.CHARACTER_LIMIT} characters.",
                )
                valid = False
    ...
    def _get_stripped_data(raw_data):
        """Strip leading/trailing whitespace from all field values before
        saving."""
        filtered_data = {
    ...
    def _determine_update_route(self, new_data):
        """Determine whether to use full replace (PUT) or partial update
        (PATCH) API route based on changed fields."""
        filtered_data = self._get_stripped_data(new_data)

        # If all values are changed, use PUT (full replace), otherwise use
    ...
    def get_entry_data(self):
        ...
        for field, widget in self._text_widgets.items():
            if isinstance(widget, Text):
                ...
                # End at the end minus 1 character to exclude trailing
                # newline added by Text widget.
